...
The objective of my research on Alzheimer's Disease (AD) is to develop advanced machine learning models that enable the early detection of cognitive decline, focusing on the prodromal stages of AD. By leveraging multimodal data, including speech, language, and acoustic features, my work aims to identify interpretable markers of cognitive changes that can serve as reliable indicators of the disease. The methodology involves preprocessing raw speech data to extract acoustic features such as pitch variation and pauses, combining them with linguistic features like syntactic complexity, derived from pre-trained large language models (LLMs). This integrative methodology not only enhances the accuracy of early AD detection but also provides interpretable insights into disease progression, empowering clinicians with tools for early intervention and better patient management. By combining longitudinal analysis with cutting-edge AI techniques, my research strives to bridge the gap between computational methods and impactful healthcare solutions.
